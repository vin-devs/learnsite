...
export default function CatalogPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Type filter
    if (filters.type !== "all") {
      products = products.filter((product) => product.kind === filters.type)
    }
    ...
    if (filters.level.length > 0) {
      products = products.filter(
        (product) => product.kind === "book" || filters.level.includes((product as Course).level),
      )
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-4">Browse All Products</h1>
          <p className="text-muted-foreground mb-6">Discover our complete collection of courses and books</p>
